...
@Injectable()
export class GithubTrendingSource {
  private readonly log = new Logger(GithubTrendingSource.name);

  async fetchTrending(options: FetchTrendingOptions = {}): Promise<TrendingRepo[]> {
    ...
    const repos = this.parse(html);
    this.log.log(`GitHub Trending (${since}): ${repos.length} repo`);
    return repos;
  }
  ...
  private parse(html: string): TrendingRepo[] {
    ...
    $('article.Box-row').each((_, el) => {
      ...
      rows.push({ owner, name, slug, url: `https://github.com${href}`, description, language, starsToday, totalStars });
    });
