...
describe('UnretweetExecutor', () => {
  ...
  it('returns errorClass=auth on AuthRequiredError', async () => {
    const xDirect = mockXDirectWrite({ unretweetTweet: jest.fn().mockRejectedValue(authError()) });
    const exec = new UnretweetExecutor(fakeRegistry(), xDirect);
  ...
  it('returns errorClass=transient on generic errors', async () => {
    const xDirect = mockXDirectWrite({ unretweetTweet: jest.fn().mockRejectedValue(new Error('boom')) });
    const exec = new UnretweetExecutor(fakeRegistry(), xDirect);
  ...
  it('registers itself on bootstrap', () => {
    ...
    const exec = new UnretweetExecutor(registry, mockXDirectWrite());
