...
@Injectable()
export class TweetRetweetersExtractionStrategy extends ExtractionStrategyBase {
  readonly type = 'tweet_retweeters' as const;
  ...
  fetch({ params, limit, accountId, cursor }: ExtractionFetchArgs) {
    return this.reads.getTweetRetweeters(this.requireTweetUrl(params), limit, accountId, cursor, {
