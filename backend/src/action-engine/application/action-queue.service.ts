...
export class ActionQueueService {
  ...
  /**
   * Age of the oldest pending action per type, in seconds. A high value means
   * the claim worker cannot keep up. Per-type reporting keeps isolated stalls
   * visible instead of hiding them in an aggregate.
   */
  ...
  /**
   * Aggregates dead actions across all action tables for DLQ views. If `type`
   * is provided, only that table is queried.
   */
